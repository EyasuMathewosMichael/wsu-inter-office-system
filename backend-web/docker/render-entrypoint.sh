...
ROOT_APP_DIR="${CATALINA_HOME}/webapps/ROOT"
LEGACY_APP_DIR="${CATALINA_HOME}/webapps/backend-web"
SERVER_XML="${CATALINA_HOME}/conf/server.xml"
PORT_VALUE="${PORT:-10000}"
UPLOAD_BASE_DIR="${UPLOAD_BASE_DIR:-/var/data/wsu-iocs}"
SEED_IMG_DIR="/opt/render-seed/assets-img"

mkdir -p "${ROOT_APP_DIR}/WEB-INF" "${LEGACY_APP_DIR}/WEB-INF"
...
for APP_DIR in "${ROOT_APP_DIR}" "${LEGACY_APP_DIR}"; do
cat > "${APP_DIR}/WEB-INF/mail.properties" <<EOF
...
EOF
    ...
    rm -f "${ROOT_APP_DIR}/WEB-INF/mail.properties" "${LEGACY_APP_DIR}/WEB-INF/mail.properties"
fi
